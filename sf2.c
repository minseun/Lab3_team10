...
void handle_pipe(char **argv, int argc);
...
void execute_command(char *cmd) {
    ...
    int in_fd = 0;  // 기본 표준 입력
    int out_fd = 1; // 기본 표준 출력
...
void handle_pipe(char **argv, int argc) {
    ...
    if (pid1 == 0) {  // 첫 번째 자식 프로세스
        ...
        dup2(pipe_fd[1], 1);  // 표준 출력을 파이프에 연결
    ...
    if (pid2 == 0) {  // 두 번째 자식 프로세스
        ...
        dup2(pipe_fd[0], 0);  // 표준 입력을 파이프에 연결
        execvp(argv[2], &argv[2]);  // 두 번째 명령어 실행
        exit(1);
    }
